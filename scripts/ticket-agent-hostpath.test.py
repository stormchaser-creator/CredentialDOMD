...
with tempfile.TemporaryDirectory(prefix='support-hostpath-', dir='/private/tmp') as tmp:
    ...
    try:
        ...
        check('real SQL applies automated label and support stamp', sql(f"select body like 'CredentialDO Support · Automated%' from support_messages where ticket_id='{T}' and is_admin_reply") == 't' and sql(f"select agent_last_reply_at is not null from support_tickets where id='{T}'") == 't')
