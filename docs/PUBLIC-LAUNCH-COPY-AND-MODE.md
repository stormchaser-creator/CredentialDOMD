...
4. **Email approval.** New launch/email content remains subject to the owner's review before sending. The current unconditional welcome/invitation send holds and Admin behavior remain unchanged; see [Launch email owner review hold](LAUNCH-EMAIL-OWNER-REVIEW-HOLD.md). The selected second setup reminder awaits exact rendered content and recipient confirmation following an automatic approval review rejection. This public-copy change does not establish a broader approval policy for future support recipients. Guide-request delivery remains separate and its existing free-beta footer still requires owner review/correction. These email workflows are outside this public-copy change.
...
Integrated release result: **110 tests passed** (95 public-site tests plus 15 current email-hold regressions), and 11 separate dormant invitation consent/auth scenarios passed. The default-off site build, generated help/CME checks, changed JavaScript ESLint checks, syntax checks for all 17 changed modules, and `git diff --check` passed. The build retains existing SDK browser-externalization and bundle-size warnings. Browser/keyboard review is still owned by root.
