...
| Medicare Care Compare, Doctors and Clinicians | `mj5m-pzi6`, property `NPI` | `cred`, `grd_yr`, `pri_spec` and `sec_spec_1..4`, `facility_name` / `org_pac_id` / `num_org_mem`, address, `telephone_number`, `ind_pac_id`, `ind_enrl_id`. One row per practice location | CMS republishes the provider-data files on a rolling cycle. The metastore item carries the dates: at the time of writing, `modified` 2026-07-31, `released` 2026-08-13. Nothing in the code depends on a cadence |
