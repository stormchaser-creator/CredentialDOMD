...
### Dashboard observation on September 19, 2026

The operator inspected the existing development instance and observed the coupled phone sign-in/sign-up controls described above. The proposed change was not saved. Both controls were restored to their original off state and checked again. No provider plan was purchased, no SMS delivery was tested, and the frontend flag remains absent/off. A successful required-email and verified-email preservation test is still needed before saving a combined provider change or activating the client.
...
- Prove that phone-only signup is unavailable even with the provider’s coupled phone sign-in/sign-up setting: required email and verified-email checks must remain enforced, and beta invitation matching must still use verified email. This provider behavior has not yet been verified. Confirm the account UI does not expose deletion, username enrollment, or API-key creation.
...
- [Add and verify a phone](https://clerk.com/docs/guides/development/custom-flows/account-updates/add-phone) — enrollment requires verification; the observed dashboard coupling still needs a verified configuration workflow.
