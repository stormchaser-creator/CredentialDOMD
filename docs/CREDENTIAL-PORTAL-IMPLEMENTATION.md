...
- **What it shows.** Decided in SQL on every call (`credential_portal_records`, `credential_portal_scope_documents`, migration `20260925040000_credential_portal_admin_access.sql`), never from `COLLECTION_KEYS` or the app's "credential" scope. Named columns only; notes, custom fields, favorites, costs, logins, settlement amounts, reasons for leaving, specimen/orderer and requester fields and any `enc1:` value never leave. Licenses exclude driver/passport/ID types; insurance is malpractice/professional liability/tail only; health records are vaccination/TB/fit test/titer only (no drug screens, hepatitis C or HIV). Malpractice history, peer references and case log summary columns are opt-in. Custom categories only when listed on the grant. `credentialPortalView.mjs` shapes the output and drops anything it does not name.
...
- **Visits.** Each visit: open the link, request a code, verify. The session lasts up to 60 minutes (300 requests); a new visit replaces the previous session; `redeemed_at` is never set. Five wrong codes per code, fifteen in total locks the grant (the owner's Resend link unlocks it), ten code emails per grant per day, 60 seconds apart, plus the existing per-recipient and per-owner hourly limits. Unknown-token code requests are throttled per client address (HMAC key) and per token.
- **Owner actions** (Clerk bearer): `status` returns `{available}` (active, not closed, inside `CREDENTIAL_PORTAL_OWNER_PROFILES`); `create` with `kind:"standing"`, `recipientEmail`, `purpose`, `accessDays`, `allowDownload`, `sections`, `customCategories`, `requestId`; `update` with `inviteId` and any of `accessDays`, `allowDownload:false`, a narrower `sections`/`customCategories` (widening returns 409 `widening_requires_new_grant`); `resend-link` (new token; the old link, code and open visit end); `preview` with a proposed scope or an `inviteId` (the same SQL builder and shaping as the administrator's summary); `list` (status, end date, last visit, per-document activity with the owner's own document names); `revoke`. `list` and `revoke` work outside the allowlist.
- **Recipient actions** (memory bearer session): `summary` returns `{grant:{purpose,accessEndsAt,allowDownload}, physician:{name,degreeType,npi,specialties,primaryState,additionalStates,email}, sections:[{key,label,records:[{id,title,expirationDate,fields:[{label,value}],documents:[{id,name,mimeType,sizeBytes}]}]}], documentCount, expiresAt}`; `documents` (flat list); `view`; `download` (403 `download_disabled` when downloads are off, audited as `download_refused`).
- **Mail.** `request-code` answers 202 before any lookup and does the lookup, code and mail after the response (`EdgeRuntime.waitUntil`), closing the timing signal. Standing invitations and resent links are also sent after the response. The invitation names the physician ("<Name>, <Degree> has given you view access to their credential file"), the purpose and the end date, is plain text with real line breaks, and sets `reply_to` to the physician's verified email. Creation refuses (409 `profile_name_required`) without a profile name. Outbox payloads now live at most one hour (was 23).
- **Closed accounts.** Every owner check also asks `account_is_closed()`; `credential_portal_prune` deletes a closed account's grants (cascading sessions, outbox, audit) and is scheduled every five minutes by pg_cron in the migration. delete-account deletes `credential_portal_invites` by `owner_profile_id`.
