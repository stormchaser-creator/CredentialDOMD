...
export function credentialSharePayload({ files = [], subject, blurb, letter }) {
  ...
  return files.length
    ...
    : { title: scrubSsn(subject), text: scrubSsn(letter) };
}
...
export function bundleShareText(settings = {}, docs = [], date = new Date()) {
  ...
  return { title: scrubSsn(`Credential packet: ${who} (${count})`), letter: scrubSsn(letter), blurb: scrubSsn(blurb) };
}
...
export function veraPacketShareText(coverNote) {
  ...
  return { title: "Credential packet", note: scrubSsn(`${body}\n\nSent from CredentialDOMD`), blurb: scrubSsn(blurb) };
}
