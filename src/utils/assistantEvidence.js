...
const REVIEW_PENDING = new Set(['AZ:DO', 'VT', 'WA', 'MO']);
...
function ruleEvidence(rule, jurisdiction, degree) {
  return {
    ...
    citation: rule.source || null, url: https(rule.sourceUrl),
    ...
    topics: (rule.topics || []).map(t => ({
      ...
      url: https(t.url || rule.sourceUrl),
...
3. Where reviewStatus is needs_independent_review, explicitly flag the stored rule conflict; offer the board/source link and a practical next step, without asserting the disputed total as current law. For other references distinguish stored guidance from current authoritative confirmation. Unknown effective dates do not establish a rule for a historical or future cycle. For historical cycles describe the saved calculation and ask for the period's board instructions before giving a legal conclusion. Do not silently move a past renewal date into the future.
