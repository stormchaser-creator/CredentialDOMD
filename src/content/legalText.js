    ...
    },
    {
      ...
      blocks: [
        [
          ...
          "**A read-only support view, only if you allow it.** In Settings you can allow support to view your account for 24 hours, and end that at any time. While it is allowed, the service administrator can open a read-only view of your account in the app for up to 15 minutes at a time and only after entering a reason. This view is separate from the administrative access to the database and storage described above. Nothing can be added, changed, deleted, sent or shared from that view. Passport and travel IDs, tax, invoice, expense and deduction records, contract rates and terms, portal logins and Protected Identity are never shown, and files are opened one at a time. Each view and each file opened is logged with the reason, and you can read that log in Settings.",
    ...
    },
    {
      ...
      blocks: [
        [
          ...
          "**More > Data Rights > Delete All My Data** clears this device and deletes everything the app holds for your account on the server: your records, uploaded documents, monthly backups and their archives, support tickets and their screenshots, feedback, notes from the operator and your replies to them, the assistant log, AI usage rows, error reports, and any support access you allowed with its log. Your profile is reduced to an account id with no name, email, or other fields, so the sign-in still resolves. It cannot be undone; export first.",
