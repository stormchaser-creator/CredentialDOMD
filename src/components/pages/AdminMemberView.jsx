...
import { createMemberViewClient, formatWhen } from "../../utils/memberViewClient.js";
...
export default function AdminMemberView({ target, grant, onClose }) {
  ...
  const reasonOk = trimmed.length >= 10 && trimmed.length <= 500;
  ...
  const start = async () => {
    ...
    try { setOpened(await client.start({ profileId: target.id, reason: trimmed })); }
    catch (failure) { setError(failure?.message || "The support view could not be opened."); }
    finally { inFlight.current = false; setBusy(false); }
  };

  if (opened) {
    return <MemberViewer opened={opened} client={client} T={T} isDesktop={isDesktop}
  ...
  return <Modal open onClose={onClose} title="View as member" width={560}>
    <div style={{ color: T.text, fontSize: 14, lineHeight: 1.6 }}>
      <p style={{ marginTop: 0 }}>
        See <strong>{name}</strong>&apos;s account the way they see it, read-only. Nothing can be added, changed, deleted, sent or shared.
      </p>
      <ul style={{ paddingLeft: 18, margin: "0 0 12px", color: T.textMuted }}>
        ...
        <li>Passport and travel IDs, taxes, invoices, expenses, deductions, contract rates and Protected Identity are never shown.</li>
      </ul>
      ...
      <div style={{ fontSize: 12, color: T.textMuted, marginTop: 4 }}>{trimmed.length} of 500 characters, at least 10.</div>
