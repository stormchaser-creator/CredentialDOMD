...
function AppInner({ tab, setTab, subPage, setSubPage }) {
        ...
        <div style={{
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
            {missingExpiration.slice(0, 5).map(({ item, sec, label }) => {
              ...
              return (
              <button key={item.id} onClick={() => { setTab("credentials"); setSubPage(sec); setAutoEditTarget({ sec, id: item.id, focus: "expirationDate" }); }} style={{
        ...
        <div style={{ marginBottom: 20 }}>
          ...
          <div className="cmd-snap-scroll">
            {urgent.slice(0, 6).map(item => {
              ...
              return (
                <div key={item.id} onClick={() => { setTab("credentials"); setSubPage(item._sec); setAutoEditTarget({ sec: item._sec, id: item.id, focus: "expirationDate" }); }} style={{
  ...
  const renderCredentials = () => {
    ...
    if (subPage === "healthRecords") return <HealthRecordsSection onShare={openShare} autoEditId={autoEditTarget?.sec === "healthRecords" ? autoEditTarget.id : null} autoFocusField={autoEditTarget?.sec === "healthRecords" ? autoEditTarget.focus : null} onAutoEditDone={() => setAutoEditTarget(null)} />;
