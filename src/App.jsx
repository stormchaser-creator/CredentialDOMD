...
function AppInner({ tab, setTab, subPage, setSubPage, navRecord }) {
  ...
  const renderCredSection = (sub) => {
    ...
    ]} items={[...(data.travelDocs || [])].sort((a, b) => (a.type || "").localeCompare(b.type || "") || (a.name || "").localeCompare(b.name || ""))} {...crud("travelDocs")} onShare={openShare} emptyIcon={"✈️"} emptyTitle="No travel records" emptySub="Passports, driver's licenses, Known Traveler Number, loyalty programs, rental memberships, airline credits. The numbers every new assignment asks for." fields={[
      { key: "type", label: "Type", type: "select", options: ["Airline credit", "Airline loyalty", "Driver\u2019s License", "Global Entry", "Hotel loyalty", "Known Traveler (TSA PreCheck)", "Passport", "Rental car membership", "Visa", "Other"] },
