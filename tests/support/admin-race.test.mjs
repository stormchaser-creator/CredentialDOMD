...
function fixture(){
  ...
  const imports={react,'react/jsx-runtime':{jsx:(type,props,key)=>({type,props,key}),jsxs:(type,props)=>({type,props})},'../../context/AppContext':{useApp:()=>({theme:{},user:{id:'synthetic-owner'},data:{},userIdRef:{current:'owner'}})},'../../lib/supabase':{supabase:db},'../../lib/admin':{useIsAdmin:()=>true},'../../utils/adminSupportThread':{loadAdminSupportThread(_client,id){const d=deferred();reads.push({...d,id});return d.promise;}},'../../utils/edgeError':{edgeErrorMessage:async()=> 'Synthetic failure'},'../../utils/ticketAttachments':{attachmentsPayload:()=>({}),linksFor:x=>x||[]}};
