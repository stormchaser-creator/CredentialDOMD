...
const root = () => {
  const actions = [{textContent:'Membership signup'}], statuses = [{textContent:'Check availability in app.'}];
  const nodes = Object.fromEntries(['headline','price','price-label','heading','phase'].map(key=>[key,[{textContent:''}]]));
...
test('each current-offer element follows all three server phases and failures remove the previous price', async () => {
  ...
  for (const phase of ['founding','earlybird','standard']) {
    ...
    assert.equal(dom.nodes.heading[0].textContent,`${label} membership signup`);
  ...
  assert.equal(dom.nodes.price[0].textContent,'Check in app');
  assert.doesNotMatch(JSON.stringify(dom.nodes),/\$99|\$149|\$199/);
});
...
test('public phase updates use only validated annual prices and distinguish capacity holds from a sales pause', () => {
  for (const [phase,amount] of [['founding',99],['earlybird',149],['standard',199]]) {
    assert.equal(offerPresentation(offer(phase)).action, `Review $${amount}/year offer`);
  }
...
test('public GET sends no identity or authorization and never caches or redirects', async () => {
  ...
  assert.match(result.status,/does not reserve a place/);
...
test('unknown configuration and failed status clear stale display instead of retaining or inventing $99 availability', async () => {
  ...
  await refresh();assert.match(dom.actions[0].textContent,/149/);
  await refresh();assert.equal(dom.actions[0].textContent,'Review membership offers');
  assert.equal(dom.statuses[0].textContent,'Check availability in app.');
...
test('a stale earlier public response cannot replace a later phase and all rendering uses text', async () => {
  ...
  assert.equal(dom.actions[0].textContent,'Review $149/year offer');
